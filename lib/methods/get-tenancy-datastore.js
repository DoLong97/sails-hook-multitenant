  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (req && req.constructor.name === "IncomingMessage") {
      ...
      if (_datasource.search(config.identity)) {
        resolve(sails.hooks.orm.datastores[config.identity]);
      } else {
      ...
      if (_datasource.search(hash)) {
        resolve(sails.hooks.orm.datastores[hash]);
      } else {
        await createDatasourceFromAdapter(adapters[req.adapter], req, hash, req.schema, function() {
          adapters[req.adapter].datastores[hash].name = hash;
          sails.hooks.orm.datastores[hash] = adapters[req.adapter].datastores[hash];
          resolve(adapters[req.adapter].datastores[hash]);
        });
      }
